...
const actions = [
  ...
  },
  {
    ...
    description:
      'Gets the received signal strength indicator (RSSI) of the current network in dBm. Android only.',
    inputs: [],
    run: async (values) => {
  ...
  },
  {
    ...
    label: 'Get WiFi info',
    description:
      'Gets comprehensive information about the currently connected WiFi network including SSID, BSSID, IP, frequency, link speed, and signal strength in a single call.',
    inputs: [],
    run: async (values) => {
  ...
  },
  {
    ...
    description:
      'Starts scanning for available WiFi networks. Android only. Results delivered via networksScanned event.',
    inputs: [],
    run: async (values) => {
  ...
  },
  {
    ...
    description:
      'Connects to a WiFi network. Creates temporary connection on Android, persistent on iOS.',
    inputs: [
      { name: 'ssid', label: 'SSID', type: 'text', value: '' },
      { name: 'password', label: 'Password (optional)', type: 'text', value: '' },
      { name: 'isHiddenSsid', label: 'Hidden network (Android)', type: 'checkbox', value: false },
      {
        name: 'autoRouteTraffic',
        label: 'Auto-route traffic (Android)',
        type: 'checkbox',
        value: false,
      },
    ],
  ...
  },
  {
    ...
    label: 'Add network',
    description:
      'Shows system dialog to add a WiFi network (Android SDK 30+) or connects directly (iOS).',
    inputs: [
      { name: 'ssid', label: 'SSID', type: 'text', value: '' },
      { name: 'password', label: 'Password (optional)', type: 'text', value: '' },
      { name: 'isHiddenSsid', label: 'Hidden network (Android)', type: 'checkbox', value: false },
  ...
  },
  {
    ...
    description:
      'Disconnects from the current WiFi network. On iOS, only disconnects networks added via this plugin.',
